    ...
    try {
      ...
      (() => {
        buttons.forEach((item, i) => {
          $(`${this.containerSelector} #toolbar .main-buttons`).append(`<button id="${item.name}" data-i="${i}">${item.icon}</button>`);
        })
        ...
        $(`button`).click(function () {
          alert('alert');
          let id = $(this).attr('id');
          $(`#toolBar #toolBarOutBox  .toolBarInBox button`).removeClass('active');
          $(`#toolBar #toolBarOutBox  .toolBarInBox button#${id}`).addClass('active');
          _self.setActiveTool(id);
        })       
          ...
          if (id === 'save') {
            ...
            let name ,key;
             for (let i=0; i< localStorage.length; i++) {
               key = localStorage.key("img_"+i);
             // console.log(`localStorage ${key}`);
            } 
            name = 'img_'+Number(parseInt(key.split('_')[1])+1);
           // console.log(name);
              // saveInBrowser.save('canvasEditor', _self.canvas.toJSON());
             saveInBrowser.save(name, _self.canvas.toJSON());
            }
          } else if (id === 'clear') {
            if (window.confirm('This will clear the canvas! Are you sure?')) {
              _self.canvas.clear(), saveInBrowser.remove(name);
            }
          } else if (id === 'download') {
