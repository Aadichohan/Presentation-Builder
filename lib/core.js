...
(function () {
  ...
  var ImageEditor = function (containerSelector, buttons, shapes) {
    ...
    // this.containerEl = document.getElementById(containerSelector);
    this.containerEl = document.querySelector(containerSelector);
// using add method
// adding single class
  this.containerEl.classList.add('default-container');
...
console.log('this.containerEl ',this.containerEl);
   ...
   // this.containerEl.addClass('default-container');
    ...
    //   var toolpanel = document.querySelector(`${containerSelector} .toolpanel`);
    //   toolpanel.classList.remove('visible');
      ...
      $(`${containerSelector} .toolpanel#${id}-panel`).addClass('visible');
        // var toolpanelId = document.querySelector(`${containerSelector} .toolpanel#${id}-panel`);
        //     toolpanelId.classList.add('visible')
        if (id === 'select') {
          $(`${containerSelector} .toolpanel#${id}-panel`).attr('class', `toolpanel visible type-${this.activeSelection.type}`)
         //toolpanelId.setAttribute('class', `toolpanel visible type-${this.activeSelection.type}`)
        }
      }
     ...
     */
    this.initializeMainPanel = () => {
        const main_panal = document.createElement('div')
        main_panal.setAttribute('class','main-panel');
        document.querySelector(containerSelector).appendChild(main_panal);
        //$(`${this.containerSelector}`).append('<div class="main-panel"></div>');
    }
